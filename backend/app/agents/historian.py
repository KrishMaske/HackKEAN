...
def historian_agent(state: AgentState) -> AgentState:
    ...
    if not year:
        state["reasoning_log"].append("Historian Agent: No year found in historical context")
        return state
    
    if not proposed_objects:
        state["reasoning_log"].append("Historian Agent: No proposed objects to validate")
        return state
    ...
    if not state.get("guardrails_enabled", True):
        ...
        state["reasoning_log"].append(f"Historian Agent: GUARDRAILS DISABLED. Letting '{final_selection}' pass through without validation.")
        return state
...
Provide your analysis for each object."""
    ...
    if valid_objects:
        ...
        reasoning = f"Historian Agent: '{final_selection}' validated as historically accurate for {year}"
    elif substitutes:
        final_selection = substitutes[0]
        reasoning = f"Historian Agent: Original object anachronistic, substituted with '{final_selection}' from {year}"
    else:
        # Fallback: use first proposed object if no clear validation
        final_selection = proposed_objects[0]
        reasoning = f"Historian Agent: Using '{final_selection}' as fallback (could not validate)"
    
    # Update state
    state["final_selection"] = final_selection
    state["reasoning_log"].append(reasoning)
    state["reasoning_log"].append(f"Historian Agent: Gemma 4 response: {gemma_response}")
